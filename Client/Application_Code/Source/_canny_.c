...
{
    //bitmap_info_header_t* bitmapInfoHeader = (bitmap_info_header_t*)malloc(sizeof(bitmap_info_header_t));
    ...
    if (*((uint16_t*)mag.magic) != 0x4D42) {
        FreeRTOS_printf(("Not a BMP file: magic=%c%c\n",
    ...
    if (bitmapInfoHeader->compress_type != 0)
        FreeRTOS_printf(("Warning, compression is not supported.\n"));
                     ...
                     const int nx, const int ny, const float sigma)
{
    ...
    float *kernel= (float*)malloc(n * n * sizeof(float));  // variable length array
 
    //FreeRTOS_printf(("gaussian_filter: kernel size %d, sigma=%g\n",n, sigma));
    size_t c = 0;
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
                              ...
                              const float sigma)
{
    ...
    if (G == NULL || after_Gx == NULL || after_Gy == NULL ||
        nms == NULL || out == NULL) {
        FreeRTOS_printf(("canny_edge_detection:"
...
int canny_filter(const int argc, const char ** const argv)
{
    if (argc < 2) {
        FreeRTOS_printf(("Usage: %s image.bmp\n", argv[0]));
        return 1;
    }
    ...
    if (in_bitmap_data == NULL) {
        FreeRTOS_printf(("main: BMP image not loaded.\n"));
        return 1;
    }
    ...
    if (out_bitmap_data == NULL) {
        FreeRTOS_printf(("main: failed canny_edge_detection.\n"));
        return 1;
    }
 
    if (save_bmp("out.bmp", &ih, out_bitmap_data)) {
        FreeRTOS_printf(("main: BMP image not saved.\n"));
        return 1;
    }
